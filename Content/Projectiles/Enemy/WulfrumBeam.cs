...
namespace CalamityRelics.Content.Projectiles.Enemy
{
    public class WulfrumBeam : ModProjectile
    {
        ...
        {
            NPC probe = Main.npc[OwnerNPC];
            if (!probe.active || probe.type != ModContent.NPCType<WulfrumProbe>())
            {
                Projectile.Kill();
                return;
            }
            
            Projectile.rotation = probe.rotation;
            Projectile.Center = probe.Center + Projectile.rotation.ToRotationVector2() * 50f;
